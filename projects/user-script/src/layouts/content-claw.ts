...
function isExcludedElement(el: Element): boolean {
  ...
  while (cur) {
    if (
      cur.id === 'selective-crawl-floating-root' ||
      cur.id === 'scwc-notify-root' ||
      cur.classList.contains('scw-floating-window')
    ) {
      return true;
    }
...
class SCWCContentClaw extends LitElement {
  ...
  /** 是否正在选择元素 */
  @state()
  private accessor selecting = false;
  ...
  /** 选中元素的 selector 列表 */
  @state()
  private accessor items: Item[] = loadFromStorage<ArrayProcessor<Item>>(
    SELECTIVE_CRAWL_KEY,
    (item) => {
  ...
  /** 选中元素的描述输入 */
  @state()
  private accessor descInput = '';

  /** 选中元素的前缀输入 */
  @state()
  private accessor prefixInput = '';
        ...
        >
          ...
          ${this.selectedEl
            ? html`
              ...
              `
            : nothing}
          ${!this.selecting && !this.selectedEl
            ? html`<scwc-button
                type="secondary"
                ?active=${this.selecting}
                title="点击触发抓取数据"
                            ...
                            <span
                              ...
                              @mouseenter=${() => {
                                ...
                                if (this.highlightEl) {
                                  ...
                                  const highlightEl = document.createElement(
                                    'scwc-element-highlight',
                                  ) as SCWCElementHighlight;
