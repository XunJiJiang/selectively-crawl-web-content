import { PERSISTENT_DATA_KEY, POS_KEY, MINIMIZED_KEY, PLUGIN_EXPANDED_KEY } from "./common"

/** 更新本地存储的数据格式 */
export default function updateLocalStorage () {
  const result = updateLocalStorageFormat_0_to_2_0_0_1()
  if (result === 'error') {
    console.error('0 -> 2.0.0-1 更新本地存储数据格式时发生错误, 请手动清除 localStorage 中的相关数据')
    return
  }
  // 其他更新逻辑可以在这里添加
}
...
function updateLocalStorageFormat_0_to_2_0_0_1 (): UpdateResult {
  ...
  if (persistentData) {
    try {
      ...
      console.error('解析 PERSISTENT_DATA_KEY 时发生错误: ' + (error instanceof Error ? error.message : String(error)))
      return 'error'
    }
  }
