...
export async function sendCrawlRequest (data: SCWC.TDataItem[], config: TConfig): Promise<TCrawlResponse> {
  try {
    ...
    if (resData && resData.success === false) {
      return { success: false, message: resData.message || '抓取失败', data: resData.data ?? [] };
    }
    ...
    return { success: false, message: `抓取失败: ${e instanceof Error ? e.message : String(e)}`, data: [] }
  }
}