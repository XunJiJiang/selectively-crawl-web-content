...
export function isConfigItem(item: any): item is ScriptConfigItem {
  return (
    item &&
    typeof item === 'object' &&
    item['script-config-symbol'] === CONFIG_SYMBOL &&
    typeof (item as ScriptConfigItem).trigger === 'function'
  );
}

export type { TConfig } from '../types/config.d.ts';
...
export class ConfigController implements ReactiveController {
  static config: TConfig = loadFromStorage(
    CONFIG_KEY,
    defaultValue as unknown as JSONValueWithFunction,
    'a',
  );
  static allControllers = new Set<ConfigController>();
  /** 同样是所有控制器的 Set, 但是使用 WeakSet */
  static allControllersWeak = new WeakSet<ConfigController>();
  /** 清理 Set 中已经不存在的控制器 */
      ...
      },
      {
        ...
        trigger: () => {
          ...
          notify({
            ...
            description:
              '这是一条测试通知这是一条测试通知这是一条测试通知这是一条测试通知这是一条测试通知这是一条测试通知这是一条测试通知',
            type: 'warn',
  ...
  setConfig(newConfig: TConfig) {
    ...
    saveToStorage(CONFIG_KEY, newConfig as unknown as JSONValue);
