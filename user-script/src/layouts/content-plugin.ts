      ...
      >
            ...
            (plugin) => html`
              <div
                ...
                @click=${() => {
                  ...
                  this.dispatchEvent(
                    new CustomEvent('current-plugin-tab-changed', {
                      detail: plugin.id,
                    }),
                  );
                }}
        ...
        <div class="plugin-window-content">
          ${activeTab && activePlugin
            ? activePlugin.controls.map((control) => {
                switch (control.type) {
                  case 'button':
                    return html`
                      <scwc-button
                  ...
                  case 'toggle':
                    return html`
                      <scwc-checkbox
                        ...
                        title=${control.label}
                        aria-label=${control.label}
                        @change=${(e: SCWCCheckboxEventMap['change']) =>
                          this.pluginsController.setControlValue(control, e.detail)}
                      ></scwc-checkbox>
                    `;
                  case 'select':
                    return html`
                      <scwc-select
                        ...
                        @change=${(e: SCWCSelectEventMap['change']) =>
                          this.pluginsController.setControlValue(control, e.detail)}
                      ></scwc-select>
                    `;
                  case 'input:text':
                    return html`
                      <scwc-input
                        ...
                        aria-label=${control.label}
                        value=${this.pluginsController.getControlValue(control)?.toString()}
                        @change=${(e: SCWCInputEventMap['change']) =>
                          this.pluginsController.setControlValue(control, e.detail)}
                      ></scwc-input>
                    `;
                  case 'input:number':
