...
export function matchLink(link: string, patterns: string[]): boolean {
  ...
  const wildcardPatterns = patterns.filter(
    (pattern) => pattern.includes('*') || pattern.startsWith('!'),
  );
  const nonWildcardPatterns = patterns.filter(
    (pattern) => !pattern.includes('*') && !pattern.startsWith('!'),
  );
  const root = link;
  // 统一去除尾部斜杠
  ...
  let isStartsWithMatch = false;
  {
    ...
    // 尝试直接匹配
    const isDirectMatch =
      nonWildcardPatterns.some((link) => unifiedRoot.startsWith(link)) ||
      nonWildcardPatterns.length === 0;
    if (isDirectMatch) {
      isStartsWithMatch = true;
    }
    // 尝试匹配去除协议后的链接
    const isMatchWithoutProtocol =
      nonWildcardPatterns.some((link) => rootWithoutProtocol.startsWith(link)) ||
      nonWildcardPatterns.length === 0;
